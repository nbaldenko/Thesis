...
# that represents the electric circuit that is being modeled
        # genfiles = ['MyPVGenfile_1.DSS', 'MyPVGenfile_2.DSS', 'MyPVGenfile_3.DSS']

        if filename != "":
            DSStext.Command = "compile " + filename
            DSStext.Command = "New EnergyMeter.Feeder Line.HVMV_Sub_connector 1"
            DSStext.Command = "Distribute kW=1000 how=random PF=1 file=MyPVGenfile.DSS"
            DSStext.Command = "Set Maxiterations=200" 
...
#       print "PV Generation:", around(self.circuit.Generators.kW)
...
# note:  all three methods get called on any pick event
        ...
        self.fig = fig
        self.selected, = plot(x[0:3], y[0], 'o', ms=25, alpha=5,
...
#        ax.set_yticks([123.6])
#        ax.set_yticklabels(['1.03 p.u.'])
        ax.plot([0, x.max()],[120*1.03, 120*1.03],'k-',lw=2)
...
# of the actual points on the plots
        ...
        connect('pick_event', self.print_branch_info)
        self.mapfig = fig
# plot a red circle at the 'to' bus of the line segment if clicked on with
# the mouse
        self.mapselected,  = plot([x2[0]], [y2[0]], 'o', ms=25, alpha=5, color='red', visible=False)
...
if __name__ == '__main__':
    #kWs = ['100', '500','1000','5000']
    #for kW in kWs:
        d = DSS(r"C:\Users\nicolasbaldenko\Documents\OpenDSS\IEEETestCases\8500-Node\Master.dss")

        ### save the results to disk
        file_name = r"C:\Users\nicolasbaldenko\Documents\OpenDSS\dss_object.p"
